#!/usr/bin/perl
# version 1.2
# author: steffen lott - steffen.lott@biologie.uni-freiburg.de
...
sub fasta2lumberjack{
    ...
    foreach my $f (@seqFiles){
        unless ( ($f eq ".") || ($f eq "..") ){
            ...
            # new: RNAfold -> RNAdistance instead of RNApdist
            my $resultRNAdistance = computeRNAdistance($fastaSeqs);
            #print($resultRNAdistance);
            my ($dist_path) = rnaPdist2matrix($resultRNAdistance);
            #print($dist_path);
...
# FUNCTIONS
# ---------------------------------------------------------------------
=head2 computeRNAdistand

 Title   : computeRNAdistance
 Usage   : my $rna_distance_dist_matrix = computeRNAdistance($fasta_file);
...
sub computeRNAdistance{
    my $fastaSeqs = $_[0];
	## execute RNAfold + RNAdistance from vienna package - make the tool available in your path variables!
	my $resultRNAdistance = `RNAfold < $fastaSeqs \| RNAdistance -Xm`;
    #print($resultRNApdist[0:-1]);
	## clean output files because they are not needed for the further steps
	system("rm .\/*.ps");
    #print ($resultRNApdist . "\n");
	return $resultRNAdistance;
}
