...
"""
GLASSgo Version 1.5.1
NEW FEATURE: gi -> acc REPLACEMENT
"""
        ...
        if line:
            ...
            if float(line[6]) <= e_value:
                ...
                if len(lines) >= 2:
                    ...
                    coord1 = int(s_pos_genome)
                    coord2 = int(e_pos_genome)
                    #id_taxID = str(lines[0]) + "-taxID:" + str(line[7])
                    def_text = lines[0].split(" ", 1)[1]

                    if coord1 < coord2:
                        ori = "+"
                        id_taxID = ">" + str(acc) + ":" + str(coord1) + "-" + str(coord2) + " " + str(def_text) + "-taxID:" + str(line[7])
                        # id, sequence, coord1, coord2, ori, identity
                        array.append([id_taxID, "".join(lines[1:]), coord1, coord2, ori, 0])
                    else:
                        ori = "-"
                        id_taxID = ">" + str(acc) + ":c" + str(coord1) + "-" + str(coord2) + " " + str(def_text) + "-taxID:" + str(line[7])
                        # id, sequence, coord1, coord2, ori, identity
...
def blast_seed(in_seek_seq, db, e_value, acc_list, num_hits, n_threads):
    # compile the call
    format_str = "\"6 qseqid saccver qstart qend sstart send evalue staxids\""
    if acc_list is None:
        ...
        blastn_cline = NcbiblastnCommandline(cmd='blastn', task='blastn', query=in_seek_seq,
                                             db=db, evalue=e_value, outfmt=format_str, seqidlist=acc_list, max_target_seqs=num_hits, num_threads=n_threads)
...
def call_londen(input_fasta_file, in_directly_stored_lum, area_filter, cutting_method, londen_cl_ids_entries, londen_scaling_mode):
    ...
    #act_args = shlex.split("perl " + str(scriptPath) + "/reqPackages/londen_modified.pl -f " + str(input_fasta_file) + " -t " + str(area_filter) + " -m " + str(cutting_method))
    act_args = shlex.split(str(scriptPath) + "/reqPackages/londen_modified -f " + str(input_fasta_file) + " -t " + str(area_filter) + " -m " + str(cutting_method))
    p = subprocess.Popen(act_args, stdout=PIPE)
    (output, err) = p.communicate()
    p.wait()
...
if __name__ == "__main__":
    ...
    parser.add_argument("-g", "--acc_pos", help="allowed ACC list", type=str)
    ...
    blast_result = blast_seed(input_filename, args.db, args.eValue, args.acc_pos, args.max_target_seqs, args.num_threads)
    ...
    for i in range((args.blast_runs - 1)):
        ...
        blast_result = blast_seed(tmp_fasta_file_name, args.db, args.eValue, args.acc_pos, args.max_target_seqs, args.num_threads)
        fasta_hits = extent_reads_from_blast_table(input_filename, blast_result, args.db, args.eValue)
        fasta_hits_rated = global_alignment(input_filename, fasta_hits)
        (directly_stored_high_sim, directly_stored_tmp, for_reblast) = analyze_sequences(fasta_hits_rated, new_min_identity,
                                                                                         args.max_identity, args.secure_mode,
